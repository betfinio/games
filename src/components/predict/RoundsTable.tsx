...
import {Game, Round, RoundStatus} from "@/src/lib/predict/types";
...
function RoundInfo({round, game, theme}: { round: number, game: Game, theme: 'dark' | 'light' }) {
	...
	if (round === 0 || roundData === null) return <div className={cx('h-[60px] w-full animate-pulse rounded-xl', theme === 'light' ? 'bg-primaryLight' : 'bg-primary')}></div>
	...
	return <Link to={'/predict/' + game.name + '?round=' + round}
	             className={cx('w-full rounded-xl text-sm md:text-base cursor-pointer flex flex-row items-center md:gap-4 justify-between md:px-4 px-2 h-[60px]', theme === 'light' ? 'bg-primaryLight' : 'bg-primary')}>
		...
		<div className={'flex md:w-[160px] flex-row justify-between items-center rounded-lg bg-secondaryLighter p-2'}>
			...
			<div
				className={cx('w-[80px] flex flex-row items-center justify-center gap-1 rounded-md', end > start ? 'bg-green-600 ' : 'bg-opacity-30 text-red-500 bg-red-900')}>
				{end}
		...
		<div className={'flex h-[36px] md:h-[40px] flex-row w-[100px] md:w-[200px] text-white items-center rounded-md overflow-hidden'}>
			...
			<div className={' px-1 py-[6px] h-full bg-opacity-30 bg-red-900 text-red-500 flex flex-row items-center gap-1 justify-end'}
		...
		<div className={'text-gray-300 w-[80px] hidden md:block'}>
			{t('roundStatuses.' + status)}
		</div>
...
export const TableTimer: FC<{ roundId: number, currentRoundId: number, className?: string, duration?: number }> = ({roundId, className = '', duration = 4}) => {
	...
	return <div className={cx('flex w-full', className)}>
