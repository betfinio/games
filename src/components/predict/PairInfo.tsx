...
const PairInfo: FC<{ game: Game }> = ({game}) => {
	...
	return <div
		className={'h-[80px] border border-gray-800 w-full whitespace-nowrap bg-primaryLight rounded-lg px-4 sm:px-6 py-4 flex flex-row items-center justify-start gap-2 sm:gap-4 md:gap-6 relative'}>
		...
		<div className={'grow flex justify-end items-center gap-4 lg:gap-8'}>
			...
			<div className={'hidden lg:flex flex-col'}>
				...
				<div
					className={cx('font-semibold flex flex-row items-center gap-1', {'animate-pulse blur-sm': !isVolumeFetched})}>
		...
		<div className={cx('flex flex-row justify-between items-center gap-4 sm:gap-8 px-6')}>
			...
			<a target={'_blank'} href={"https://betfin.gitbook.io/betfin-public/v/games-manual/games-guide/predict-game"}
