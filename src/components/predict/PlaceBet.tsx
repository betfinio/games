...
const PlaceBet: FC<{ game: Game }> = ({game}) => {
	...
	return <div className={'flex flex-col gap-4 col-span-4 md:col-span-3 items-center drop-shadow-[0_0_35px_rgba(87,101,242,0.75)]'}>
		<h2 className={'font-medium uppercase hidden md:block'}>{t('title')}</h2>
		<div
			className={cx('w-full border border-gray-800 rounded-[10px] bg-primaryLight py-5 px-10 flex flex-col items-center gap-6 relative')}>
			...
			<div className={'grid grid-cols-2 gap-3 w-full'}>
				<motion.button whileHover={{scale: 1.03}} whileTap={{scale: 0.97}} onClick={() => handleBet(true)}
				               disabled={loading || amount === null || Number(amount) < 1 || BigInt(amount) * 10n ** 18n > balance}
				               className={' rounded-lg flex flex-col items-center pb-2 p-3 gap-2 leading-[0px] justify-between bg-green-500 font-semibold  disabled:cursor-not-allowed disabled:grayscale duration-500'}>
				...
				<motion.button whileHover={{scale: 1.03}} whileTap={{scale: 0.97}} onClick={() => handleBet(false)}
				               disabled={loading || amount === null || Number(amount) < 1 || BigInt(amount) * 10n ** 18n > balance}
				               className={' rounded-lg flex flex-col items-center pb-2 p-3 gap-2 leading-[0px] justify-between bg-red-500 font-semibold disabled:cursor-not-allowed disabled:grayscale duration-500'}>
...
const CoefficientRatio: FC<{ pool: RoundPool, amount: string | number }> = ({pool, amount}) => {
	...
	return <div
		className={'flex  text-xs flex-row w-full text-white items-center rounded-md overflow-hidden'}>
		<div
			className={'px-3 py-1 h-full bg-opacity-30 bg-green-900 text-green-500 flex flex-row items-center gap-1'}
...
const PlayersBets: FC<{ game: Game }> = ({game}) => {
	...
	return <div className={'hidden md:grid grid-cols-2 gap-4 w-full'}>
		<div
			className={'bg-primary rounded-lg p-2 flex justify-center gap-2 items-center text-green-500 font-semibold'}>
			<ArrowUpIcon className={'h-3 w-3'}/>
			<div
				className={cx('flex flex-row gap-1 items-center text-sm', {'animate-pulse blur-sm': !isBetsFetched})}>{millify(valueToNumber(userPool.long), {precision: 2})}
				<Bet className={'w-3 h-3'} color={'green'}/></div>
		</div>
		<div
			className={'bg-primary rounded-lg p-2 flex justify-center gap-2 items-center text-red-500 font-semibold'}>
			<ArrowDownIcon className={'h-3 w-3'}/>
			<div
				className={cx('flex flex-row gap-1 items-center text-sm', {'animate-pulse blur-sm': !isBetsFetched})}>{millify(valueToNumber(userPool.short), {precision: 2})}
	...
	return <div className={'grid grid-cols-2 gap-4 w-full'}>
		<TooltipProvider delayDuration={0}>
			<Tooltip>
				...
				<TooltipContent>
					{(expected.longBonus + expected.long).toFixed(2) + ' BET'}
				</TooltipContent>
			</Tooltip>
			<Tooltip>
				<div className={'bg-primary rounded-lg p-2 flex flex-col justify-center items-center '}>
					<TooltipTrigger>
						<div className={'w-full flex flex-row justify-center gap-1 text-red-500 font-semibold'}>
							<ArrowDownIcon className={'h-3 w-3'}/>
							<div
								className={cx('flex flex-row gap-1 items-center', {'animate-pulse blur-sm': isBetsFetched})}>{millify(expected.short, {precision: 2})}
					...
					<TooltipContent>
						{(expected.shortBonus + expected.short).toFixed(2) + ' BET'}
					</TooltipContent>
				</div>
			</Tooltip>
		</TooltipProvider>
