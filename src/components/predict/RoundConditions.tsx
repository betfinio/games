...
import {defaultResult, Game} from '@/src/lib/predict/types';
...
const RoundConditions: FC<{ game: Game }> = ({game}) => {
	...
	return <div className={'flex flex-col gap-4 items-center col-span-4 md:col-span-3 md:col-start-2 lg:col-start-1 relative'}>
		<h2 className={'font-medium uppercase'}>{t('title')} #{round.toString().slice(2)}</h2>
		<div
			className={cx('w-full border border-gray-800 rounded-[10px] bg-primaryLight p-2 py-5', isFetching && "animate-pulse blur-sm")}>
			...
			<div className={'flex flex-col items-center relative justify-center gap-4 w-[285px] aspect-square mx-auto'}>
				...
				<div className={'text-center flex flex-col gap-1'}>
					<h4 className={'font-medium text-xs text-gray-500'}>
						<Trans values={{time: DateTime.fromMillis(Number(start.timestamp) * 1000).toFormat('HH:mm')}}
						       i18nKey={'games.predict.roundConditions.price'}
						       i18n={i18n}
						       components={{b: <b className={'text-yellow-400 font-semibold'}/>}}/>
					</h4>
					<div
						className={cx('rounded-[10px] mt-1 text-center text-sm bg-primary px-4 py-2 font-semibold text-yellow-400', {'animate-pulse blur-sm': !isLatestPriceFetched})}>
				...
				<div className={'text-center flex flex-col gap-1'}>
					...
					<div className={cx('rounded-[10px] w-full text-center text-sm bg-primary px-4 py-2 font-semibold', price.answer > start.answer ? ' text-green-500' : 'text-red-500')}>
			...
			<div className={'flex justify-center  text-gray-500 text-xs mt-6'}>
				You bet if the rate goes <span className={'text-green-500 px-1'}> UP </span> or <span
				className={'text-red-500 px-1'}>DOWN</span> till <span
				className={'text-yellow-400 pl-1'}> {DateTime.fromMillis((round + 4) * game.interval * 1000).toFormat("TT")}</span>
			</div>
		</div>
	</div>
...
export const Clock: FC<{ game: Game, className?: string }> = ({
	game: {interval},
}) => {
	...
	return <CircularProgressbar styles={progressStyle} counterClockwise={true}
	                            value={timer / interval * 100}
	/>
	
}

export const Timer: FC<{ game: Game, className?: string, size?: string }> = ({
	game: {interval},
	className = '',
}) => {
	...
	return <div className={cx('text-3xl', className)}>
