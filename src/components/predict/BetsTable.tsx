...
const BetsTable: FC<{ isFetching: boolean, bets: PredictBet[], isFinished: boolean }> = ({isFetching, isFinished, bets}) => {
	...
	const columns = [
		...
		columnHelper.accessor("side", {
			...
			cell: (props) => <span className={cx('font-semibold text-xs md:text-base', props.getValue() ? 'text-green-500' : "text-red-500")}>
		...
		columnHelper.accessor("result", {
			...
			cell: (props) => {
				...
				return <span
					className={cx('font-medium', res === 0 ? 'text-gray-500' : 'text-green-500')}>
		...
		columnHelper.accessor("bonus", {
			...
			cell: (props) => <span className={cx('font-medium text-yellow-400')}>
				{valueToNumber(props.getValue()!)} BET
			</span>
		}),
		columnHelper.display({
			id: 'total',
			header: t('table.total')!,
			cell: (props) => <span className={'font-medium'}>
				{props.row.original.status === 5n ? t('table.statuses.5') :
		...
		columnHelper.accessor("status", {
			...
			})}>{t(`table.statuses.${props.getValue().toString() as '0' | '1' | '2' | '3' | '4'}`)}</span>
		}),
	]
	...
	const renderTableSkeleton = () => {
		return [...Array(5)].map((_, i) => (
			<tr key={i}
			    className={cx('h-[50px] p-0 ', i % 2 ? 'bg-primaryLight' : 'bg-primaryLighter')}>
				{[...Array(columns.length)].map((_, i) => (
					<td key={i}
					    className={cx('text-left px-2')}>
					</td>
				))}
			</tr>
		))
	...
	const renderTableBody = () => {
		...
		return table.getRowModel().rows.filter(filterBets).map(row => (
			<tr key={row.id}
			    className={cx('h-[50px] rounded-xl p-0 ', row.index % 2 ? 'bg-primaryLight' : 'bg-primaryLighter')}>
				{row.getVisibleCells().map(cell => (
					<td key={cell.id}
					    className={cx('text-left px-2', cell.column.columnDef.meta?.className, isFinished && cell.column.id === 'status' && 'hidden', !isFinished && ['bonus', 'result', 'total'].includes(cell.column.id) && 'hidden')}>
	...
	return (<>
			<div className={'flex flex-row justify-start items-center gap-4'}>
				<button onClick={() => setFilter('all')}
				        className={cx('px-4 py-2 rounded-lg w-[120px]', filter === 'all' ? 'bg-yellow-400 text-black' : 'bg-primaryLighter')}>{t('table.filter.all')}</button>
				<button onClick={() => setFilter('my')}
				        className={cx('px-4 py-2 rounded-lg w-[120px]', filter === 'my' ? 'bg-yellow-400 text-black' : 'bg-primaryLighter')}>{t('table.filter.my')}</button>
			</div>
					...
					{table.getHeaderGroups().map(headerGroup => (
						<tr key={headerGroup.id} className={'text-gray-400 text-left'}>
							{headerGroup.headers.map(header => (
								<th key={header.id}
								    className={cx('h-[40px] pl-2', header.column.columnDef.meta?.className, isFinished && header.id === 'status' && 'hidden', !isFinished && ['bonus', 'result', 'total'].includes(header.id) && 'hidden')}>
