	...
	return <DialogContent className={'games text-white'}>
		<motion.div onClick={(e) => e.stopPropagation()}
			...
			<div className={'flex flex-row gap-2 justify-start items-center'}>
				<div className={'flex flex-col gap-1 md:w-1/3 md:whitespace-nowrap'} onClick={handleCalculate}>
					<div className={'text-2xl flex items-center gap-5'}>
						<span>{t("title")} #{round.toString().slice(2)}</span>
						{isFinished && (isLong ? (
							<div className={'text-2xl font-bold text-green-500 flex items-center gap-1'}>
								Long
								({coef === Infinity ? 1 : coef.toFixed(2)}x)
							</div>
						) : (
					...
					<span className={'-mt-1'}>{start.toFormat("dd.MM.yyyy / HH:mm")} - {end.toFormat("HH:mm")}</span>
				
				</div>
				{!isFinished &&
					<RoundTimer game={game} end={(round + game.duration) * (game.interval)} className={'!justify-start'}
					            size={'60px'}/>}
			...
			{isFinished && (
				<div className={'flex flex-col gap-2 md:flex-row md:gap-10 mt-2 items-start'}>
					...
					<div className={'flex flex-col gap-2'}>
							<span className={' flex gap-2'}>Total bonus:
								<BetValue value={valueToNumber(totalBonus)} withIcon={true}
					...
					<div className={' gap-5 md:gap-1 flex md:flex-col'}>
						...
						<div><span className={'text-green-500'}>Long</span> / <span
							className={'text-red-500'}>Short</span> ratio:
						</div>
						<div
							className={'flex h-[36px] md:h-[40px] flex-row w-[200px] text-white items-center rounded-md overflow-hidden'}>
							<div
								className={'px-1 py-[6px] h-full bg-opacity-30 bg-green-900 text-green-500 flex flex-row items-center gap-1'}
								style={{width: (valueToNumber(roundData.pool.long) / (valueToNumber(roundData.pool.long) + valueToNumber(roundData.pool.long)) * 100) + '%'}}>
			...
			<div className={'grid grid-cols-3 gap-4 my-4'}>
				<div
					className={' bg-primaryLight md:col-span-1 col-span-3 rounded-lg p-4 flex flex-row md:flex-col items-center  justify-between gap-2 py-4'}>
					<People className={'w-8 h-8 md:w-14 md:h-14'}/>
					<h2
						className={'font-bold text-xl md:text-2xl'}>{t('users', {players})} / {t('bets', {bets: bets.length})}</h2>
			...
			<div className={cx('flex flex-row justify-end w-full p-2', (status !== 'ended') && "!hidden")}>
				<button className={'rounded-md px-4 py-2 bg-yellow-400 text-black '} onClick={handleCalculate}>Calculate
					Result
				</button>
			</div>
		</motion.div>
...
export const RoundTimer: FC<{ game: Game, end: number, className?: string, size?: string }> = ({
	game: {interval},
	end,
	className = '',
	size = '100px'
	...
	return <div className={cx('flex items-center gap-2 px-6 w-full justify-center', className)}>
		<CircularProgressbar styles={progressStyle} counterClockwise={true}
		                     value={timer.mins / (interval / 60) * 100}
