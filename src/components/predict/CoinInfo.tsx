...
const CoinInfo: FC<{ bets: number; volume: bigint; staking: bigint; diff: number }> = ({
	bets,
	volume,
	staking,
	diff
...
const SwitchModal: FC<{ bets: number; volume: bigint; staking: bigint; diff: number }> = ({
	bets,
	volume,
	staking,
	diff
	...
	return <motion.div className={'rounded-lg border border-gray-800 bg-primary p-5 w-[300px] flex flex-col gap-5'}>
		<div className={'flex justify-between'}>
			<span className={'text-sm'}>{t('tile.24change')}</span>
			<span className={cx('font-semibold text-sm lg:text-lg', {
