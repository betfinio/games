...
const RouletteWheel: FC = () => {
	...
	useEffect(() => {
		if (wheelState.state === 'standby') {
			anime.set([".ROULETTE"], {
				rotate: function () {
					return -getRotationFromNumber(lastNumber);
				}
			});
		}
	}, [isBetsFetched]);
	...
	function spinWheel() {
		...
		anime.set([".ROULETTE"], {
			rotate: function () {
				return -getRotationFromNumber(tmp);
			}
		});
		anime({
			targets: [".ROULETTE"],
	...
	async function stopWheel(result: number, bet: Address) {
		...
		const totalRotation = -(Math.floor(Math.random() * (wheelMaxNumberOfSpins - wheelMinNumberOfSpins + 1) + wheelMinNumberOfSpins) * 360 + singleRotationDegree * getRouletteIndexFromNumber(result) + randomOffset);
	...
	return <>
		<div className={'block'}>
			<motion.div initial={{marginTop: '-30%'}} animate={{
			...
			}} transition={{duration: 1}} className={'flex items-center justify-center overflow-hidden relative pb-20'}>
				<div
					className={cx('rounded-full w-4/5 md:w-2/3 aspect-square lg:drop-shadow-[0_0_35px_rgba(0,172,231,0.45)] bg-roulette-wheel bg-cover ROULETTE', {'blur-md animate-pulse': !isBetsFetched})}></div>
