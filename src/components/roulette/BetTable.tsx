...
import {FuncProps} from "@/src/lib/roulette/types.ts";
import {Tooltip, TooltipContent, TooltipProvider, TooltipTrigger} from "betfinio_app/tooltip";
import ActionModal from "@betfinio/ui/dist/shared/modal/ActionModal";
import {ETHSCAN} from "@/src/global.ts";

const RouletteBetTable: FC = () => {
	return <div className={'grid grid-cols-3 md:grid-cols-1 gap-1 md:gap-2 '}>
		<TableDesktop/>
	</div>
}
...
const items: Record<number, number[]> = {
	...
	1065: arrayFrom(36).map(e => e + 1).filter(e => e % 2 === 0),
	1066: arrayFrom(36).map(e => e + 1).filter(e => e % 2 === 0),
	1067: arrayFrom(36).map(e => e + 1).filter(e => e % 2 === 0),
	1077: arrayFrom(36).map(e => e + 1).filter(e => e % 2 === 1),
	1078: arrayFrom(36).map(e => e + 1).filter(e => e % 2 === 1),
	1079: arrayFrom(36).map(e => e + 1).filter(e => e % 2 === 1),
	56: arrayFrom(36).map(e => e + 1).filter(e => e % 3 === 0),
	114: arrayFrom(36).map(e => e + 1).filter(e => e % 3 === 2),
	172: arrayFrom(36).map(e => e + 1).filter(e => e % 3 === 1),
}
...
const TableDesktop = () => {
	...
	const selected = bets.map(e => e.numbers).flat()
	const selectedItems = bets.map(e => e.item).flat()
	...
	return <div>
		<div className={'hidden lg:grid row-start-1 grid-cols-3'}>
			<div className={'col-span-2 my-3 relative'}>
				<Additional className={'w-[800px] max-w-full h-auto'} selected={selected} highlight={highlight}
				            onClick={handleAdditionalClick} onHover={handleAdditionalHover}
				            onMouseLeave={handleAdditionalMouseLeave}/>
			</div>
			
			<div className={'col-span-1 flex items-center justify-center'}>
				<motion.button whileHover={{scale: 1.05}} whileTap={{scale: 0.95}}
				               className={'rounded-md flex-grow flex justify-center lg:flex-grow-0 lg:rounded-lg bg-yellow-400 border-4 border-transparent text-black text-4xl px-10 py-4 font-semibold pointer disabled:bg-transparent disabled:text-gray-800 disabled:border-gray-800 disabled:cursor-not-allowed transition-all duration-300'}
		...
		`}>
			...
			`}>
				<div
					className={cx(greenClass, 'col-span-3 lg:col-span-1 lg:row-span-3', highlight.includes(0) && highlightClass, selected.includes(0) && selectedClass)}>0
				</div>
				{nums.map((item) => <motion.div key={item} className={cx('aspect-square',
					selected.includes(item) && selectedClass,
					highlight.includes(item) && highlightClass,
					getColor(item) === "RED" ? redClass : blackClass)}>
					{item}
				</motion.div>)}
				{arrayFrom(3).map((item) => <div key={item + 100} className={cx(helperClass, 'aspect-square')}>1:2</div>)}
			</div>
			...
			`}>
				<div className={'col-span-1 row-span-1 aspect-square'}></div>
				<div
					className={cx(helperClass, 'row-span-4 row-start-2 lg:row-span-1 lg:row-start-1 lg:col-span-4 lg:col-start-2 vertical-rl lg:horizontal-tb', (highlightCombination >= 1003 && highlightCombination <= 1009) && highlightClass, selectedItems.includes(1006) && selectedClass)}>1
					to 12
				</div>
				<div
					className={cx(helperClass, 'row-span-4 row-start-6 lg:row-span-1 lg:row-start-1 lg:col-span-4 lg:col-start-6 vertical-rl lg:horizontal-tb', (highlightCombination >= 1011 && highlightCombination <= 1017) && highlightClass, selectedItems.includes(1014) && selectedClass)}>13
					to 24
				</div>
				<div
					className={cx(helperClass, 'row-span-4 row-start-10 lg:row-span-1 lg:row-start-1 lg:col-span-4 lg:col-start-10 vertical-rl lg:horizontal-tb', (highlightCombination >= 1019 && highlightCombination <= 1025) && highlightClass, selectedItems.includes(1022) && selectedClass)}>25
					to 36
				</div>
				<div
					className={cx(helperClass, 'row-span-2 row-start-2 col-start-2 lg:row-span-1 lg:row-start-2 lg:col-span-2 lg:col-start-2 vertical-rl lg:horizontal-tb', (highlightCombination >= 1061 && highlightCombination <= 1063) && highlightClass, selectedItems.includes(1062) && selectedClass)}>1
					to 18
				</div>
				<div
					className={cx(helperClass, 'row-span-2 row-start-4 col-start-2 lg:row-span-1 lg:row-start-2 lg:col-span-2 lg:col-start-4 vertical-rl lg:horizontal-tb', (highlightCombination >= 1065 && highlightCombination <= 1067) && highlightClass, selectedItems.includes(1066) && selectedClass)}>Even
				</div>
				<div
					className={cx(redClass, 'row-span-2 row-start-6 col-start-2 lg:row-span-1 lg:row-start-2 lg:col-span-2 lg:col-start-6 vertical-rl lg:horizontal-tb', (highlightCombination >= 1069 && highlightCombination <= 1071) && highlightClass, selectedItems.includes(1070) && selectedClass)}>Red
				</div>
				<div
					className={cx(blackClass, 'row-span-2 row-start-8 col-start-2 lg:row-span-1 lg:row-start-2 lg:col-span-2 lg:col-start-8 vertical-rl lg:horizontal-tb', (highlightCombination >= 1073 && highlightCombination <= 1075) && highlightClass, selectedItems.includes(1074) && selectedClass)}>Black
				</div>
				<div
					className={cx(helperClass, 'row-span-2 row-start-10 col-start-2 lg:row-span-1 lg:row-start-2 lg:col-span-2 lg:col-start-10 vertical-rl lg:horizontal-tb', (highlightCombination >= 1077 && highlightCombination <= 1079) && highlightClass, selectedItems.includes(1078) && selectedClass)}>Odd
				</div>
				<div
					className={cx(helperClass, 'row-span-2 row-start-12 col-start-2 lg:row-span-1 lg:row-start-2 lg:col-span-2 lg:col-start-12 vertical-rl lg:horizontal-tb', (highlightCombination >= 1081 && highlightCombination <= 1083) && highlightClass, selectedItems.includes(1082) && selectedClass)}>19
					to 36
				</div>
			</div>
			<TableDesktopControls onHighlight={(value) => {
...
const TableDesktopControls: FC<TableDesktopControlsProps> = ({onHighlight}) => {
	...
	const handleClick = (item: number) => {
		...
		if (arrayFrom(8).map(e => e + 1002).includes(item)) item = 1006;
		if (arrayFrom(7).map(e => e + 1011).includes(item)) item = 1014;
		if (arrayFrom(7).map(e => e + 1019).includes(item)) item = 1022;
		if (arrayFrom(3).map(e => e + 1061).includes(item)) item = 1062;
		if (arrayFrom(3).map(e => e + 1065).includes(item)) item = 1066;
		if (arrayFrom(3).map(e => e + 1069).includes(item)) item = 1070;
		if (arrayFrom(3).map(e => e + 1073).includes(item)) item = 1074;
		if (arrayFrom(3).map(e => e + 1077).includes(item)) item = 1078;
		if (arrayFrom(3).map(e => e + 1081).includes(item)) item = 1082;
		...
		return (
			<Tooltip>
				...
				<TooltipContent className={'bg-primary !bg-opacity-100 flex flex-col gap-1 p-4 text-white text-sm rounded-md'}>
					...
					<p>
						Potential win: <span
						className={'font-bold text-green-400'}>{(total * (36 / chip[0].numbers.length)).toLocaleString()} BET</span>
					</p>
					<p>
						Combination: <span className={'font-bold text-yellow-400'}>{getCombination(chip[0].item)}</span> <span
						className={'text-green-400 text-xs'}>({36 / chip[0].numbers.length}x)</span>
					</p>
	...
	return <TooltipProvider delayDuration={0}>
		...
		<div className={'absolute bottom-[4.5%] left-[-1.8%] w-[103.5%] grid grid-cols-29 grid-rows-3'}>
			{arrayFrom(87).map(e => e + 1000).map((item) => <div onClick={() => handleClick(item)} onContextMenu={(e) => {
				handleContextMenu(item, e)
			}} key={item} onMouseLeave={clear} onMouseEnter={() => highlight(item)}
			                                                     className={cx(chipClass, 'relative')}>{renderChip(item)}</div>)}
		</div>
	</TooltipProvider>
}
