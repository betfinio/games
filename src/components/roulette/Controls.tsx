...
const RouletteControls = () => {
	...
	return <div
		className={'lg:border border-gray-800 lg:bg-primaryLighter gap-1 lg:gap-3 lg:p-4 lg:my-4 rounded-xl flex-grow w-1/2 lg:w-full  flex-row flex-wrap lg:items-center items-end justify-start lg:justify-between  md:flex'}>
		<Dialog>
			...
			<div className={'flex lg:hidden w-full'}>
				<div className={'flex-grow flex-row items-center flex'}>
					<motion.div whileTap={{scale: 0.95}}
					            className={'aspect-square p-2 px-4 flex flex-row items-center rounded-lg justify-center bg-primary cursor-pointer text-yellow-400 font-medium text-xl'}
					            onClick={handleMinus}>
						-
					</motion.div>
					<Slider className={cx('mx-3', {'blur-sm animate-pulse': !isLimitsFetched})} styles={{
						rail: {
							backgroundColor: '#333  !important',
						},
						handle: {background: '#facc15', border: '#0088cc'}
					}} value={valueToNumber(bigValue)} dotStyle={{background: '#10121D', borderColor: '#facc15', fontSize: '8px'}}
					        min={limits.min}
					        max={limits.max} step={limits.max / 100} included={false} marks={marks} onChange={handleChange}/>
					<motion.div whileHover={{scale: 1.05}} whileTap={{scale: 0.95}}
					            className={'p-2 px-4 flex flex-row items-center rounded-lg justify-center gap-2 bg-primaryLight cursor-pointer'}>
						<DialogTrigger>
							<motion.div className={'font-normal text-xs whitespace-nowrap w-[70px] '}>
								{millify(valueToNumber(bigValue))} BET
							</motion.div>
						</DialogTrigger>
						<span className={'text-yellow-400 font-medium text-xl'} onClick={handlePlus}>+</span>
					</motion.div>
				</div>
			</div>
			<motion.button whileHover={{scale: 1.05}} whileTap={{scale: 0.95}}
			               className={'rounded-md flex-grow h-[40px] flex justify-center lg:flex-grow-0 lg:rounded-lg bg-yellow-400 text-black px-6 py-2 font-semibold pointer disabled:grayscale disabled:pointer-events-none'}
			...
			<motion.button onClick={handleDouble} whileHover={{scale: 1.05}} whileTap={{scale: 0.95}} className={'rounded-lg text-yellow-400 bg-primaryLight px-6 py-2 font-semibold'}>
				Double
			</motion.button>
			<div className={'flex-grow flex-row items-center hidden lg:flex'}>
				<motion.div whileTap={{scale: 0.95}}
				            className={'aspect-square w-[40px] h-[40px] p-2 px-4 flex flex-row items-center rounded-lg justify-center bg-primary cursor-pointer text-yellow-400 font-medium text-xl'}
				            onClick={handleMinus}>
					-
				</motion.div>
				<Slider className={cx('mx-3', {'blur-sm animate-pulse': !isLimitsFetched})} styles={{
					...
					handle: {background: '#facc15', border: '#0088cc'}
				}} value={valueToNumber(bigValue)} dotStyle={{background: '#10121D', borderColor: '#facc15'}} min={limits.min}
				        max={limits.max} step={limits.max / 100} included={false} marks={marks} onChange={handleChange}/>
				<DialogTrigger>
					<motion.div
						className={'font-normal border rounded-full border-transparent hover:border-yellow-400 p-1 duration-300 cursor-pointer relative whitespace-nowrap w-20  flex flex-row items-center  gap-1'}>
						<Chip className={cx('duration-300', {
						...
						})} value={valueToNumber(bigValue)} fontSize={11}/>
					</motion.div>
				</DialogTrigger>
				<motion.div whileTap={{scale: 0.95}}
				            onClick={handlePlus}
			...
			<motion.button whileHover={{scale: 1.05}} whileTap={{scale: 0.95}} onClick={handleUndo}
			               className={'rounded-lg h-[40px] text-yellow-400 bg-primaryLight p-3 lg:px-6 py-2 font-semibold flex flex-row gap-2 justify-center items-center'}>
...
const ChangeBetModal: FC<{ initialValue: number, max: number, min: number }> = ({initialValue, max, min}) => {
	...
	const handleChange = (e: any) => {
		...
		if ((inputValue === '' || Number.isInteger(parseFloat(inputValue))) && inputValue >= 0) {
			setValue(inputValue);
		}
	}
	const handleSave = () => {
	...
	return <motion.div layoutId={'chip'} className={'bg-primary rounded-lg p-3 flex flex-col gap-2 text-white'}>
		<h2 className={'text-sm text-gray-500'}>Custom amount of chip:</h2>
		<div className={'relative'}>
			<input type="number" min={0} className={'rounded-lg p-2 px-4 bg-primaryLighter border border-gray-800 pr-10'}
			       value={value} onChange={handleChange}/>
			<span className={'absolute right-2 top-[9px]'}>BET</span>
		</div>
		<DialogClose>
			<div className={'flex flex-row justify-between'}>
				<button className={'bg-red-roulette rounded-lg px-4 p-2 w-2/5'} onClick={handleClose}>Cancel</button>
				<button className={'bg-green-500 rounded-lg px-4 lg-2 w-2/5'} onClick={handleSave}>Save</button>
			</div>
		</DialogClose>
	</motion.div>
}