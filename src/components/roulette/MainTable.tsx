...
const RouletteTable = () => {
	...
	const renderLastResults = () => {
		return lastSeven.map((result, index) => (
			<div
				key={index}
				className={cx(
					'aspect-square text-white rounded-lg flex justify-center items-center ',
					{
	...
	const renderStat = () => {
		return (
			<div className={cx('flex flex-col items-center justify-start gap-1', { 'blur-sm animate-pulse': !isBetsFetched })}>
				...
				<div className={'grid grid-cols-2 grid-row-3 gap-1 w-4/5 grid-flow-col'}>
					{hot.map((item, index) => (
						<div key={index} className={'aspect-square bg-red-600 rounded-lg flex items-center justify-center col-start-1'}>
							{item}
						</div>
					))}
					{cold.map((item, index) => (
						<div key={index} className={'aspect-square bg-blue-600 rounded-lg flex items-center justify-center col-start-2'}>
							{item}
						</div>
					))}
				</div>
				...
				<div className={'grid grid-cols-2 grid-row-1 gap-1 w-4/5 grid-flow-col'}>
					<div className={'aspect-square bg-red-600 text-xs rounded-lg flex items-center justify-center col-start-1'}>{red}%</div>
					<div className={'aspect-square bg-gray-800 text-xs rounded-lg flex items-center justify-center col-start-2'}>{black}%</div>
				</div>
				<span className={'font-semibold text-sm'}>Odd/Even</span>
				<div className={'grid grid-cols-2 grid-row-1 gap-1 w-4/5 grid-flow-col'}>
					<div className={'aspect-square bg-blue-600 text-xs rounded-lg flex items-center justify-center col-start-1'}>{odd}%</div>
					<div className={'aspect-square bg-green-600 text-xs rounded-lg flex items-center justify-center col-start-2'}>{even}%</div>
				</div>
			</div>
		);
	};
	...
	return (
		<div className={'grid grid-rows-1 gap-4 h-full grid-cols-8 items-start overflow-hidden'}>
			<div className={'bg-primaryLight rounded-lg p-2 mt-4 border border-gray-800 '}>
				...
				<div className={cx('grid grid-cols-3 grid-rows-7 gap-1', { 'blur-sm animate-pulse': !isBetsFetched })}>{renderLastResults()}</div>
			</div>
			<div className={'col-span-6 row-span-3 '}>
				<Wheel />
			</div>
			<div className={'col-span-1 bg-primaryLight rounded-lg row-span-1 p-2 block mt-4 border border-gray-800'}>{renderStat()}</div>
		</div>
	);
};
